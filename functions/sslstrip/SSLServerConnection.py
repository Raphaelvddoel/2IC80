...
class SSLServerConnection(ServerConnection):
    ...
    def handleHeader(self, key, value):
        ...
        encoded_changed_value = value
        if (decoded_key.lower() == 'set-cookie'):
            # CHANGE: decoded value
            decoded_value = value.decode('utf-8')
            changed_value = SSLServerConnection.cookieExpression.sub("\g<1>", decoded_value)
            encoded_changed_value = changed_value.encode('utf-8')

        ServerConnection.handleHeader(self, key, encoded_changed_value)

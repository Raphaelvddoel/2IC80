...
# CHANGE: Url parse should be imported from urllib 
...
#        self.uniqueId      = random.randint(0, 10000)
    ...
    def cleanHeaders(self):
        ...
        # CHANGE: decode headers to be able to parse them
        decoded_headers = {}
        for header in headers:
            decoded_header_key = header.decode('utf-8')
            decoded_header_value = headers[header].decode('utf-8')
            
            decoded_headers[decoded_header_key] = decoded_header_value

        if 'accept-encoding' in decoded_headers:
            del decoded_headers['accept-encoding']

        if 'if-modified-since' in decoded_headers:
            del decoded_headers['if-modified-since']

        if 'cache-control' in decoded_headers:
            del decoded_headers['cache-control']

        return decoded_headers

    def getPathFromUri(self):
        # CHANGE: URI needed to be decoded
        decoded_uri = self.uri.decode('utf-8')
        
        if (decoded_uri.find("http://") == 0):
    ...
    def handleHostResolvedSuccess(self, address):
        ...
        # TO_CHANGE?: Dont send request over HTTP
    ...
    def sendExpiredCookies(self, host, path, expireHeaders):
        # CHANGE: added b to change it to bytes
        self.setResponseCode(302, b"Moved")
